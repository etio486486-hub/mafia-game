...
/** 1일차 낮(dayIndex 1)은 밤 관찰 전이라 제외; 상한 없음 — 매 낮 직업 질문 스킬 */
...
function canRunPdRoleInterview(room, helpers) {
  if (!room?.game || !helpers) {
    // #region agent log
    agentLog({
      hypothesisId: 'H1',
      location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
      message: 'pd day skill gate: no game/helpers',
      runId: 'pd-day-skill',
      data: { ok: false, reason: 'no_game_or_helpers' }
    });
    // #endregion
    return false;
  }
  const dayIdx = room.game.dayIndex || 0;
  if (dayIdx < MIN_DAY_INDEX) {
    // #region agent log
    agentLog({
      hypothesisId: 'H1',
      location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
      message: 'pd day skill gate: below min day',
      runId: 'pd-day-skill',
      data: { ok: false, reason: 'below_min_day', dayIdx, minDay: MIN_DAY_INDEX }
    });
    // #endregion
    return false;
  }
  if (isRealPoliceAlive(room, helpers)) {
    // #region agent log
    agentLog({
      hypothesisId: 'H1',
      location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
      message: 'pd day skill gate: real police alive',
      runId: 'pd-day-skill',
      data: { ok: false, reason: 'real_police_alive', dayIdx }
    });
    // #endregion
    return false;
  }
  ...
  if (!pd) {
    ...
    agentLog({
      hypothesisId: 'H1',
      location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
      message: 'pd day skill gate: no alive pd',
      runId: 'pd-day-skill',
      data: { ok: false, reason: 'no_alive_pd', dayIdx }
    });
    // #endregion
    return false;
  }
  ...
  if (bots.length < 2) {
    ...
    agentLog({
      hypothesisId: 'H1',
      location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
      message: 'pd day skill gate: not enough bot targets',
      runId: 'pd-day-skill',
      data: { ok: false, reason: 'few_bots', dayIdx, botCount: bots.length }
    });
    // #endregion
    return false;
  }
  ...
  agentLog({
    hypothesisId: 'H1',
    location: 'm42-pd-role-interview.js:canRunPdRoleInterview',
    message: 'pd day skill gate: allowed',
    runId: 'pd-day-skill',
    data: { ok: true, dayIdx, pd: pd.nickname, botCount: bots.length }
  });
  // #endregion
  return true;
}
...
function schedulePrivateDetectiveRoleInterviewWaves(room, helpers, deps) {
  ...
  if (!hasBots(room) || isRealPoliceAliveFn(room)) {
    // #region agent log
    agentLog({
      hypothesisId: 'H3',
      location: 'm42-pd-role-interview.js:schedulePrivateDetectiveRoleInterviewWaves',
      message: 'pd interview not scheduled: bots/police gate',
      runId: 'pd-day-skill',
      data: {
        hasBots: hasBots(room),
        realPoliceAlive: isRealPoliceAliveFn(room),
