...
function collectVoteIntel(room, bot, helpers) {
  ...
  try {
    ...
    if (m42Bluff.scanPoliceReporters(room, helpers).length >= 2) {
  ...
  for (const row of g.publicVoteIntel || []) {
    ...
    if (row.source === 'private_detective') {
      if (row.isMafia === true) {
        addMafia(p.id, 98);
        if (isMafia) markCleared(p.id);
      } else if (row.isMafia === false) {
        markCleared(p.id);
        addCitizen(p.id, 34 + (row.role ? powerPri(row.role) : 18));
      }
    }

    if (row.source === 'private_detective_performer' && row.role === 'private_detective') {
      markCleared(p.id);
      addCitizen(p.id, powerPri('private_detective'));
    }
    ...
    if (row.source === 'medium_dead_chat' && row.isMafia === true) {
      addMafia(p.id, 94);
      if (isMafia) markCleared(p.id);
    }

    if (row.source === 'matgyeong_probability' && row.isMafia === true) {
      addMafia(p.id, 96);
      if (isMafia) markCleared(p.id);
    }
    ...
    if (row.source === 'soldier_spy_catch' && row.isMafia === true) {
      addMafia(p.id, 102);
      if (isMafia) markCleared(p.id);
    }
    ...
    if (row.source === 'cleric_expose' && row.role === 'cult_leader') {
      addMafia(p.id, 108);
      if (isMafia) markCleared(p.id);
    }
...
function pickMafiaTeamDayVote(room, bot, helpers) {
  ...
  targets = targets.filter((p) => !isPublicConfirmedCitizenTarget(room, p.id, helpers));
...
function isStrongFactTarget(room, bot, targetId, helpers) {
  ...
  for (const row of g.publicVoteIntel || []) {
    ...
    if (row.source === 'cleric_expose' && row.role === 'cult_leader' && !isMafia) return true;
  }
...
function pickFactBasedExecutionVote(room, bot, candidate, helpers) {
  ...
  if (isPublicConfirmedCitizenTarget(room, candidate.id, helpers)) return 'no';
...
function isJatuCoordinationDay(room) {
  if (room?.game?.botMatgyeongVotePushDay) return false;
  if (room?.game?.botSoldierSpyVotePushDay) return false;
  return (
...
/** 맞경 중 진경(실제 경찰 role)이 밤에 사망하고 생존 경찰 role이 없을 때 */
function isMatgyeongAfterRealPoliceDeath(room, helpers) {
  if (room?.game?.botMatgyeongVotePushDay) return true;
  if (room?.game?.botSoldierSpyVotePushDay) return true;
  const alivePolice = helpers.getAlivePlayers
    ? helpers.getAlivePlayers(room).some((p) => p && p.role === 'police')
    : false;
  if (alivePolice) return false;
  const m42Bluff = require('./m42-bluff');
  if (m42Bluff.scanPoliceReporters(room, helpers).length < 1) return false;
  const m42MatProb = require('./m42-matgyeong-probability');
  if (m42MatProb.hasConfirmedDeadPolice && m42MatProb.hasConfirmedDeadPolice(room, helpers)) {
    return true;
  }
  const deaths = room.game?.lastNightReport?.deaths;
  if (!Array.isArray(deaths) || !deaths.length) return false;
  ...
  if (reveal?.role === 'police' && reveal.targetId != null) {
    const t = helpers.getPlayerById ? helpers.getPlayerById(room, reveal.targetId) : null;
...
function ingestVoteIntelFromChat(room, helpers) {
  try {
    ...
    if (voteIntel.ingestPrivateDetectiveReportsFromDayChat) {
      voteIntel.ingestPrivateDetectiveReportsFromDayChat(room, helpers);
    }
...
/** 공개 팩트·확직으로 시민 확정된 대상 (투표·처형 보호, 봇 관점 중립) */
function isPublicConfirmedCitizenTarget(room, targetId, helpers) {
  if (!targetId) return false;
  const target = helpers.getPlayerById ? helpers.getPlayerById(room, targetId) : null;
  if (!target || !target.alive) return false;
  if (helpers.isMafiaTeam && helpers.isMafiaTeam(target.role)) return false;
  ...
  for (const row of g.publicVoteIntel || []) {
    if (row.targetId !== targetId) continue;
    if (row.source === 'police' && row.isMafia === false) return true;
    if (row.source === 'private_detective' && row.isMafia === false) return true;
    if (row.source === 'private_detective_performer') return true;
    if (row.source === 'soldier_block' && row.role === 'soldier') return true;
  ...
  const confirmedRole = slots.confirmedById[targetId];
  if (confirmedRole && !helpers.isMafiaRole(confirmedRole)) return true;

  const dayChat = helpers.getChatMessages
    ? helpers.getChatMessages(room, 'day')
    : (room.chatLog && room.chatLog.day) || [];
  const m42Pd = require('./m42-private-detective');
  const pdReporters = m42Pd.scanPrivateDetectiveReporters(room, helpers);
  if (pdReporters.length === 1 && pdReporters[0].id === targetId) return true;
  const slotsPd = slots.confirmedById[targetId];
  if (slotsPd === 'private_detective') return true;

  return false;
}

/** 공개 팩트·조결·다수 지목으로 마피아로 의심·확정된 대상 (성직자 부활 제외) */
function isPublicMafiaSuspectTarget(room, targetId, helpers, perspectiveBot) {
  if (!targetId) return false;
  ingestVoteIntelFromChat(room, helpers);
  const g = room.game || {};
  ...
  for (const row of g.publicVoteIntel || []) {
    if (row.targetId !== targetId) continue;
    if (row.source === 'police' && row.isMafia === true) return true;
    if (row.source === 'private_detective' && row.isMafia === true) return true;
    if (row.source === 'chat_accuse' && row.isMafia === true) return true;
    if (row.source === 'soldier_spy_catch' && row.isMafia === true) return true;
    if (row.source === 'reporter' && row.role && helpers.isMafiaRole(row.role)) return true;
    if (row.source === 'medium' && row.role && helpers.isMafiaRole(row.role)) return true;
  }
  ...
  for (const msg of dayChat) {
    if (!msg?.text) continue;
    const { mafia } = parsePoliceReportFromText(room, msg.text);
    if (mafia.some((p) => p.id === targetId)) return true;
    const m42Pd = require('./m42-private-detective');
    const pdParsed = m42Pd.parseDetectiveReportFromText(room, msg.text);
    if (
      pdParsed.watch
      && pdParsed.watch.id === targetId
      && pdParsed.kind === 'mafia_kill'
    ) {
      return true;
    }
  }

  if (countRecentChatAccusers(room, targetId, helpers) >= 2) return true;
  if (g.dayTopVotedId === targetId) return true;

  if (perspectiveBot) {
    const { voteMafia } = collectVoteIntel(room, perspectiveBot, helpers);
    if (voteMafia.some((t) => t.id === targetId && t.pri >= 90)) return true;
  }
 ...
 * 마피아팀: 동료·교주 동맹·공개 확정 시민 제외 (정치인에게 표 넣는 건 가능)
 */
function isDayVoteTargetForbidden(room, voter, targetId, helpers) {
  ...
  if (isMafia) {
    ...
    if (isPublicConfirmedCitizenTarget(room, targetId, helpers)) return true;
    return false;
  }
...
function pickMatgyeongPoliceDayVote(room, bot, helpers) {
  if (helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role)) return null;
  if (
    isJatuCoordinationDay(room)
    && !hasConfirmedMafiaTarget(room, bot, helpers)
    && !isMatgyeongAfterRealPoliceDeath(room, helpers)
  ) {
    return null;
  }
  const m42MatProb = require('./m42-matgyeong-probability');
  const probTarget = m42MatProb.getRecommendedMatgyeongVoteTarget(room, bot, helpers);
  if (probTarget && probTarget !== bot.id) return probTarget;
...
function pickMatgyeongAfterRealPoliceDeathVote(room, bot, helpers) {
  if (helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role)) return null;
  if (!isMatgyeongAfterRealPoliceDeath(room, helpers)) return null;
  ...
  const m42Bluff = require('./m42-bluff');
  const bluffer = m42Bluff.getMafiaPoliceBlufferBot(room, helpers);
  const reporters = m42Bluff.scanPoliceReporters(room, helpers);
  ...
  if (bluffer && suspectIds.includes(bluffer.id) && Math.random() < 0.78) {
    return bluffer.id;
  }
...
function getAccuseReasonForTarget(room, bot, targetId, helpers) {
  ...
  for (const row of g.publicVoteIntel || []) {
    ...
    if (row.source === 'private_detective' && row.isMafia === false && !isMafia) {
      return null;
    }
    if (row.source === 'police' && row.isMafia === true && !isMafia) {
      return '공개된 조결(마피아)';
    }
    if (row.source === 'private_detective' && row.isMafia === true && !isMafia) {
      return '사립탐정 관찰(마피아 지목)';
    }
...
function isRolePublicForBot(room, bot, targetId, role, helpers) {
  ...
  for (const row of g.publicVoteIntel || []) {
    ...
    if (row.source === 'private_detective' && helpers.isMafiaRole(role) && row.isMafia === true) {
      return true;
    }
...
module.exports = {
  ...
  isMatgyeongAfterRealPoliceDeath,
  ...
  getClearedIds,
  isPlayerCleared,
  isPublicConfirmedCitizenTarget,
  isPublicMafiaSuspectTarget,
