...
function pickFactBasedDayVote(room, bot, helpers) {
  ...
  if (isMafia) {
    return pickBest(voteCitizen, cleared, true, room, bot, helpers);
  }
  return pickBest(voteMafia, cleared, false, room, bot, helpers);
}
...
function pickFallbackDayVoteTarget(room, bot, helpers) {
  ...
  if (candidates.length && candidates[0].pri >= 10) {
