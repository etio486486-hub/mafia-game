...
function collectVoteIntel(room, bot, helpers) {
  ...
  for (const [claimerId] of Object.entries(g.publicPoliceClaimIds || {})) {
    const p = aliveTarget(claimerId);
    if (p) markCleared(p.id);
  }
...
function pickBest(pool, cleared, isMafia, room, bot) {
...
function pickFactBasedDayVote(room, bot, helpers) {
  ...
  if (isMafia) {
    return pickBest(voteCitizen, cleared, true, room, bot);
  }
  return pickBest(voteMafia, cleared, false, room, bot);
}
...
function pickFallbackDayVoteTarget(room, bot, helpers) {
  ...
  const candidates = alive
    .filter((p) => !cleared.has(p.id) && !m42CultBots.isCultAlly(room, bot, p))
    .map((p) => ({
      id: p.id,
  ...
  if (candidates.length && candidates[0].pri >= 4) {
    return candidates[0].id;
  }

  const chatPick = chatSuspicion.pickTopSuspectId(room, bot, helpers, { clearedIds: cleared });
...
function parsePoliceReportFromText(room, text) {
  ...
  for (const p of Object.values(room.players)) {
    ...
    const innocentHit =
      ...
      || text.includes(`${n}님 조사했는데 마피아 아닙`)
      ...
      || new RegExp(`${n}님.{0,8}시민`).test(text);
    if (innocentHit) {
      innocent.push(p);
    }
    const mafiaHit =
