...
function reactToHumanPoliceReport(room, bot, ctx, last) {
  ...
  if ((!speaker || speaker.role !== 'police') && last && last.fromId !== bot.id && helpers.getPlayerById) {
    speaker = helpers.getPlayerById(room, last.fromId);
  }
  if (!speaker || speaker.role !== 'police' || speaker.isBot) return null;

  const parsed = voteFacts.parsePoliceReportFromText(room, text);
  const fromLast = last && last.text && last.fromId !== bot.id
    ? voteFacts.parsePoliceReportFromText(room, last.text)
    : { innocent: [], mafia: [] };
  const mafia = parsed.mafia.length ? parsed.mafia : fromLast.mafia;
  const innocent = parsed.innocent.length ? parsed.innocent : fromLast.innocent;
  if (!mafia.length && !innocent.length) return null;

  const who = speaker?.nickname || last?.from || '경찰';
