...
/** 낮 공개 채팅에서 시민팀 직업으로만 직공 (마피아·스파이·교주) */
function bluffAsCitizenByDay(bot) {
  if (bot.role === CULT_LEADER_ROLE) return true;
  return !!(helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role));
}
...
function buildRoleRollCallAnswer(room, bot, isMafia) {
  ...
  if (isMafia || bot.role === CULT_LEADER_ROLE) {
  ...
  if (role === 'doctor') {
    const claimLines = [
      `저는 ${name}입니다. 치료는 밤에만 가능합니다.`,
      `홀의입니다. 눈힐도 가능하지만 직접 말하진 않겠습니다.`,
      `저는 ${name}입니다. 밤에 살리는 쪽입니다.`
    ];
...
function wantsPoliceReport(triggerText, last, bot) {
  if (!helpers.isPoliceReportRequest) return false;
  const fromOther = last && last.fromId !== bot.id && helpers.isPoliceReportRequest(last.text);
  const fromTrigger = triggerText && helpers.isPoliceReportRequest(triggerText);
...
function reactToChatConfusion(room, bot, triggerText, last) {
  ...
  if (last && /수사\s*결과/.test(last.text) && last.fromId !== bot.id) {
...
function pickPoliceReportLine(room, bot, triggerText, last) {
  if (helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role)) {
    ...
    if (wantsPoliceReport(triggerText, last, bot)) {
  ...
  if (!wantsPoliceReport(triggerText, last, bot)) return null;
  if (bot.role === 'police') {
  ...
  return '생존 경찰이 없어 조결을 기다리기 어렵습니다.';
}
...
function generateRuleBased(room, bot, ctx = {}) {
  ...
  const onMafiaSideEarly = helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role);
  if (onMafiaSideEarly && m42Bluff.wantsMatgyeongAsk(triggerText)) {
  ...
  const onMafiaSide = helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role);
  const compactTrig = String(triggerText || '').replace(/\s+/g, '');
  if (!onMafiaSide && m42Bluff.MAT_CHAT.test(compactTrig)) {
    ...
    if (bot.role === 'doctor') {
      return '저는 시민입니다. 맞직이면 조결부터 보겠습니다. 의사 직공은 하지 않습니다.';
    }
  ...
  if (onMafiaSide) {
    ...
    return m42Bluff.pickMafiaBluffLine(room, bot, helpers);
  }
  ...
  if (CHAT_M42_CLAIM_META.test(triggerText) && !onMafiaSide && Math.random() < 0.4) {
  ...
  return isMafia
    ? m42Bluff.pickProactiveMafiaBluff(room, bot, helpers)
...
async function generateBotChat(room, bot, ctx = {}) {
  ...
  if (ctx.policeReport || wantsPoliceReport(triggerText, last, bot)) {
    const policeLine = pickPoliceReportLine(room, bot, triggerText, last);
  ...
  const skipLlmForPolice = bot.role === 'police' && wantsPoliceReport(triggerText, last, bot);
  const onMafiaSide = helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role);
  ...
  const llmChance = useGemini
    ? (onMafiaSide ? (bluffTrigger ? 0.72 : 0.58) : (bluffTrigger ? 0.22 : 0.45))
    : (onMafiaSide ? (bluffTrigger ? 0.48 : 0.38) : 0.25);
