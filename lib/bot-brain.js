...
function generateRuleBased(room, bot) {
  ...
  if (last && last.fromId !== bot.id && last.text) {
    ...
    if (mentioned.length === 0 && /누구|누가|어디|왜|뭐/.test(last.text)) {
      return `${speaker}님 질문에 답하자면, 저는 ${targetName}님이 가장 수상해요.`;
    }
...
async function generateBotChat(room, bot) {
  if (isLlmEnabled() && Math.random() < 0.22) {
    const llm = await tryLlm(room, bot);
    if (llm) return llm;
  }
