...
async function generateBotChat(room, bot, ctx = {}) {
  ...
  const dayEarly = (room.game?.dayIndex || 0) <= 2;
  const llmChance = useGemini
    ? (onMafiaSide ? (bluffTrigger && dayEarly ? 0.55 : 0.28) : (bluffTrigger ? 0.22 : 0.45))
    : (onMafiaSide ? (bluffTrigger && dayEarly ? 0.35 : 0.12) : 0.25);
