...
function messageLooksLikePoliceReport(room, msg) {
  ...
  if (policeFmt.looksLikePoliceReport(msg.text)) return true;
  const c = String(msg.text).replace(/\s+/g, '');
  return /(?:조사|수사|경찰)(?:결과|조결)/.test(c)
    && /(?:공개|나왔|올렸|말씀|확인|채팅|마피아|무죄|시민)/.test(c);
}
...
function reactToRecentPoliticianClaims(room, bot, last, onEvilSide) {
  ...
  if (bot.role === 'politician' && !onEvilSide && Math.random() < 0.88) {
    return `${joined}님 중 정치인 맞직이면 저 ${bot.nickname}입니다. 낮 2표·찬반 면역이 근거입니다. 발언을 더 맞춰 주세요.`;
  }
...
function generateBotDeadChat(room, bot, ctx = {}) {
  ...
  if (bot.role === 'medium') {
    const lines = [
      `저는 영매였습니다. 살아 있는 분들은 ${targetName}님 성불·사망자 채팅을 확인해 주십시오.`,
      `사망자 채팅입니다. ${targetName}님이 마피아 같았습니다. 영매 성불을 부탁합니다.`,
      `영매로서 단서를 남깁니다. ${targetName}님 쪽을 의심했습니다.`
    ];
    return lines[Math.floor(Math.random() * lines.length)];
  }
