...
const CHAT_M42_CLAIM = /직공|직업공개|홀경|홀의|홀군|홀탐|맞경|맞의|쓰리경/;
...
function pickAliveSuspectName(room, bot) {
...
function generateRuleBased(room, bot, ctx = {}) {
  ...
  const target = getTargetPlayer(room, bot);
  const targetName = pickAliveSuspectName(room, bot) || '누군가';
  ...
  if (helpers.isRoleClaimRequest && helpers.isRoleClaimRequest(triggerText)) {
    if (bot.role === 'police' && Math.random() < 0.55) {
      return pickRoleClaimLine(bot);
    }
    if (isMafia) {
  ...
  if (CHAT_M42_QUIET.test(triggerText) && !isMafia && Math.random() < 0.5) {
    return '조밤이면 은폐 마피아거나 짐승인간·사기꾼 가능성도 있어요.';
  }
