...
function finalizeBotLine(room, bot, text) {
  if (!text) return text;
  if (helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role) && m42Bluff.enforceBluffRoleConsistency) {
...
function reactToChatConfusion(room, bot, triggerText, last) {
  ...
  if (last && /맞경|맞경찰|짭경|진경/.test(last.text) && last.fromId !== bot.id) {
...
function generateRuleBased(room, bot, ctx = {}) {
  ...
  if (
    ...
    const a = matgyeong[0].nickname;
    const b = matgyeong[1].nickname;
    if (ni < 2) {
...
function generateBotDawnReaction(room, bot, nightReport) {
  ...
  const revealBoost = nightReport.reporterReveal ? 0.25 : 0.78;
