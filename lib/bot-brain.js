...
function buildAckFromPoliceParsed(room, bot, who, parsed) {
  ...
  if (innocent.length) {
    return `${who}님 조결대로 ${innocent[0].nickname}님은 일단 무죄로 보겠습니다.`;
  }
  return null;
}
        ...
        if (isEvilBluffBot(bot) && m42Bluff.mayMafiaTeamBotBluffPolice(room, bot, helpers)) {
          if (Math.random() < 0.72) {
            return m42Bluff.pickMatgyeongTikiTakaLine
              ? m42Bluff.pickMatgyeongTikiTakaLine(room, bot, rival, { isEvil: true, round: 0 })
              : m42Bluff.pickPoliceVersusBicker(bot.nickname, rival.nickname, true);
          }
        }
        if (bot.role === 'police' && Math.random() < 0.68) {
          return m42Bluff.pickMatgyeongTikiTakaLine
...
function getCivicLeadershipAnchor(room) {
  ...
  if (anyAlivePd && !alivePolice.length) {
    return { mode: 'private_detective', leaderId: anyAlivePd.id, confirmed: false };
  }
...
function generateRuleBased(room, bot, ctx = {}) {
  ...
  if (!onEvilSide && room.phase === 'day_chat') {
    ...
    if (leadership.mode === 'private_detective' && bot.id !== leadership.leaderId && Math.random() < 0.7) {
      return '확정 경찰이 없는 상태라 사립탐정 관찰 라인으로 따라가겠습니다. 사탐 지목 기준으로 표를 맞추겠습니다.';
    }
    if (leadership.mode === 'private_detective' && bot.id === leadership.leaderId && Math.random() < 0.78) {
      return '확정 경찰이 사망한 상태라 사탐 기준으로 주도하겠습니다. 밤 관찰 방향과 사망자를 묶어 마피아 의심 대상을 정리하고 투표를 모읍시다.';
    }
  }
  if (matchedPoliceLock && !canLeadMatchedPoliceTalk) {
  ...
  if (nightReport?.mediumPurify && Math.random() < 0.45) {
    ...
    if (helpers.isMafiaRole && helpers.isMafiaRole(mp.role)) {
...
function pickReporterRevealDayLine(room, bot, reveal) {
  ...
  return skillDlg.pickReporterRevealDayLine(brief, bot, reveal, helpers);
}
...
function generateBotDeadChat(room, bot, ctx = {}) {
  ...
  if (bot.role === 'police') {
    return `저는 경찰이었습니다. ${reason} 근거로 ${targetName}님이 수상합니다. 영매님은 이 사망자 채팅을 낮에 전달해 주십시오.`;
  }
  ...
  const lines = [
    `사망자 채팅입니다. ${reason} 근거로 ${targetName}님을 먼저 의심해 주십시오.`,
    `죽기 전 팩트로는 ${targetName}님 라인이 가장 수상했습니다.`,
    `영매님, 이 사망자 채팅을 낮에 인용해 ${targetName}님 쪽 검증을 밀어 주십시오.`
  ];
