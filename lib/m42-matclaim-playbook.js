...
function getPlayerClaimedRoles(room, playerId) {
  ...
  for (const msg of getDayChat(room)) {
    ...
    for (const [role, re] of Object.entries(ROLE_CLAIM_DETECT)) {
      if (re.test(compact) || re.test(msg.text)) roles.add(role);
    }
...
function scanMatClaimConflicts(room, helpers) {
  const claims = m42Bluff.scanRoleClaims(room, helpers);
  const conflicts = [];

  for (const role of MAT_ROLES) {
    let list = [...(claims[role] || [])];
    if (role === 'police') {
      list = mergeClaimants([list, m42Bluff.scanPoliceReporters(room, helpers)]);
    }
...
function pickMatClaimSuspectLine(room, bot, helpers, ctx = {}) {
  ...
  if (selfIn) {
    ...
    if (role === 'police') {
      ...
      if (Math.random() < 0.55) {
        return m42Bluff.pickMatgyeongTikiTakaLine(room, bot, { id: rival.id, nickname: rival.nickname }, { isEvil: evil, round: ctx.round || 0 });
      }
      return m42Bluff.pickPoliceVersusBicker(bot.nickname, rival.nickname, evil);
    }
    const mat = m42Bluff.buildRoleMatgyeongClaim(room, bot, helpers, role);
    if (mat) return mat;
...
function pickMatClaimTikiTakaLine(room, bot, helpers, opts = {}) {
  ...
  if (role === 'police') {
    return m42Bluff.pickMatgyeongTikiTakaLine(room, bot, rival, {
  ...
  if (claimants.some((c) => c.id === bot.id)) {
    let line = m42Bluff.buildRoleMatgyeongClaim(room, bot, helpers, role)
...
function pickMatClaimReactiveLine(room, bot, helpers, triggerText, last) {
  ...
  if (!m42Bluff.MAT_CHAT.test(compact) && !/맞직|쓰리경|쓰리의|쓰리군|쓰리영|쓰리기|짭경|짭의|짭군|짭영/.test(compact)) {
    return null;
  }
