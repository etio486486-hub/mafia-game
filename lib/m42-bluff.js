...
function pickContinuousEvilBluff(room, bot, helpers) {
  ...
  if (roll < 0.62 && wantsMatgyeongAsk('맞경')) {
    return buildMatgyeongCounterClaim(room, bot, helpers);
  }
  if (roll < 0.78) {
...
function buildLlmBluffHint(brief, bot) {
  ...
  if ((brief.dayIndex || 0) <= 1) {
    return '낮 1 경찰 연기: "○○님 조사했는데 마피아 아닙니다"처럼 짧고 자연스럽게. "수사 결과입니다" 같은 시스템 멘트 금지. 홀경·늦경·맞경 선동. 마피아·밤챗·접선 노출 금지. 계속 블러핑.';
  }
  return '경조결·맞경·직공마다 적극 블러핑. 가짜 조결·맞경 선동·짭경 몰기. "수사 결과입니다" 형식 금지. 마피아·밤챗·접선 노출 금지. 시민을 계속 헷갈리게.';
}
