...
/** 직공 요청 시 마피아·스파이는 시민팀 직업 위장 (가짜 조결 X) */
function buildMafiaRollCallAnswer(room, bot, helpers) {
  const citizenSide = ['citizen', 'citizen', 'soldier', 'reporter', 'medium'];
  const pool = citizenSide;
  let fake = pickMafiaBluffRole(room, bot, helpers);
  if (fake === 'police' && Math.random() < 0.55) {
  ...
  const lines = {
    ...
    police: [
      '저는 경찰입니다. 조사한 분만 조결하겠습니다.',
      '홀경입니다. 밤에 수사합니다.'
    ]
  };
  const open = buildBluffOpenLine(fake, bot, room);
  if (open) return open;
  const pick = lines[fake] || lines.citizen;
