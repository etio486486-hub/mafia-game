...
/** 진경 사망 후 마피아팀 지정 봇이 경찰 위장을 이어받음 */
function promoteMafiaPoliceBluffer(room, helpers) {
  ...
  const bluffer = mafiaBots.slice().sort((a, b) => String(a.id).localeCompare(String(b.id)))[0];
...
function getMafiaPoliceBlufferBot(room, helpers) {
  ...
  for (const b of sorted) {
    if (mayMafiaTeamBotBluffPolice(room, b, helpers)) return b;
  }
  return null;
}
...
function ensureMafiaPoliceBlufferClaim(room, helpers) {
  ...
  const bluffer = getMafiaPoliceBlufferBot(room, helpers);
  if (!bluffer) return null;
  const cur = getBotFakeClaim(room, bluffer.id, helpers);
  if (cur && cur !== 'police' && !['mafia', 'spy', 'cult_leader'].includes(cur)) {
    return bluffer;
  }
  setBotFakeClaim(room, bluffer.id, 'police', helpers, { force: true });
  return bluffer;
...
function enforceBluffRoleConsistency(text, room, bot, helpers) {
  if (!text || !helpers.isMafiaTeam || !helpers.isMafiaTeam(bot.role)) return text;
  const fc = getBotFakeClaim(room, bot.id, helpers);
  ...
  if (wantReport) {
    if (ledger?.line) {
      ...
      if (!report) {
        ...
        if (alt) {
          report = policeFmt.formatInnocentLine(alt);
