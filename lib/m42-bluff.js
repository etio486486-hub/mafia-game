...
function buildMatgyeongCounterClaim(room, bot, helpers) {
  ...
  return '저는 경찰입니다. 다른 분도 경찰이면 맞경입니다. 조결부터 맞추겠습니다.';
}

function pickProactiveMafiaBluff(room, bot, helpers) {
  const reporters = scanPoliceReporters(room, helpers);
  if (reporters.length >= 1 && Math.random() < 0.72) {
  ...
  const fakeJogyeol = a
    ? [
      `수사 결과입니다. ${a}님은 마피아가 아닙니다.`,
      `조사했습니다. ${a}님은 시민입니다. ${b || '다른 분'}님이 수상합니다.`
    ]
    : [];
  const pool = [
    ...fakeJogyeol,
    `저는 ${rl(myFake)}입니다. 곧 맞경 정리하겠습니다.`,
    buildBluffOpenLine(myFake, bot, room)
  ];
...
function reactToClaimBluff(room, bot, isMafia, triggerText, last, helpers) {
  ...
  if (reporters.length === 0 && Math.random() < 0.5) {
    setBotFakeClaim(room, bot.id, 'police', helpers);
    return buildBluffOpenLine('police', bot, room);
  }
...
function buildLlmBluffHint(brief, bot) {
  ...
  return [
    `위장 직업: ${fc}. 반드시 "저는 경찰입니다" 등 직공.`,
    '이미 홀경(조결 1명)이면 즉시 맞경: "맞경입니다. ○○님과 저 중 진경" — 홀경만 두지 말 것.',
    '가짜 조결·수사 결과도 가능. 마피아·밤챗·접선·스파이 노출 금지.'
  ].join(' ');
}
