...
function ensureAllEvilFakeClaims(room, helpers) {
  ...
  const policeBluffer = mafiaSorted.find((b) => mayMafiaTeamBotBluffPolice(room, b, helpers));

  if (policeBluffer && !realPolice) {
...
function buildRoleMatgyeongClaim(room, bot, helpers, roleKey) {
  ...
  if (cfg.stir && cfg.stir.length) {
    return cfg.stir[Math.floor(Math.random() * cfg.stir.length)];
  }
