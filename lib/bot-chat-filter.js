/**

 * 봇 채팅 출력 필터: 마피아 팀 노출·메타 발언·비공개 직업 단정 차단.

 */



const voteFacts = require('./bot-vote-facts');
const policeFmt = require('./police-report-format');
const m42Bluff = require('./m42-bluff');
const m42Cult = require('./m42-cult');
const { agentLog } = require('./debug-agent-log');



const CULT_LEAK_PATTERNS = [
  /교주입니다/,
  /저는\s*교주/,
  /나는\s*교주/,
  /제가\s*교주/,
  /교주팀/,
  /교주의\s*종소리/,
  /종소리가\s*울/,
  /포교했/,
  /포교하겠/,
  /신도입니다/,
  /신도였/
];

const MAFIA_LEAK_PATTERNS = [

  /마피아에게\s*위협/,

  /마피아에게\s*이득/,

  /마피아팀/,

  /우리\s*마피아/,

  /저희\s*마피아/,

  /마피아로서/,

  /마피아가\s*이기/,

  /마피아가\s*승리/,

  /마피아\s*입장/,

  /제거해야.*마피아/,

  /마피아.*제거/,

  /우선적으로\s*제거/,

  /시민에게\s*위협/,

  /경찰.*제거해야/,

  /경찰이\s*맞다면.*제거/,

  /위협이\s*될\s*수\s*있습니다/,

  /마피아에게\s*도움/,

  /경찰.*위협/,

  /경찰이면.*제거/,

  /경찰.*제거/,

  /제거.*경찰/,

  /마피아.*위협/

];



const MAFIA_SAFE_POLICE_LINES = [

  '퍼경일 수도 있습니다. 조결·취재 결과부터 확인하겠습니다.',

  '경찰이 맞다면 시민 편에 도움이 됩니다. 다른 분부터 살펴보겠습니다.',

  '일단 밤 사망·기자 공표부터 맞춰 보겠습니다.',

  '경찰 이야기는 들었습니다. 팩트 없이 지목하지 않겠습니다.',

  '맞경이면 조결·행적부터 봐야 합니다. 저는 시민입니다.',

  '홀경·맞경 구분해서 진경·짭경 가려야 합니다.'

];



const CITIZEN_SAFE_LINES = [

  '일단 조사·취재 결과부터 정리하겠습니다.',

  '경찰 확인이면 조결을 기준으로 가겠습니다.',

  '근거 없이 경찰을 제거하자는 말은 위험합니다.',

  '경찰 조결·기자 공표가 나온 뒤에 판단하겠습니다.',

  '조사 결과가 채팅에 없어 단정하기 어렵습니다.'

];



const ROLE_CLAIM_SAFE = [

  '조결·기자 공표부터 듣고 추리하겠습니다.',

  '경찰 조사 결과가 나오기 전에는 직업 단정은 어렵습니다.',

  '팩트 없이 ○○님 직업을 말하는 건 위험합니다. 조사·취재부터 확인하겠습니다.'

];



function leaksMafia(text) {

  if (!text) return false;

  const compact = text.replace(/\s+/g, '');

  return MAFIA_LEAK_PATTERNS.some((re) => re.test(text) || re.test(compact));

}

function leaksCult(text) {
  if (!text) return false;
  const compact = text.replace(/\s+/g, '');
  return CULT_LEAK_PATTERNS.some((re) => re.test(text) || re.test(compact));
}

function stripCultLeak(text, bot, room, helpers) {
  if (!text || !bot) return null;
  if (!leaksCult(text)) return null;
  const hideCult = bot.role === m42Cult.ROLE_CULT_LEADER
    || (bot.joinedCult && bot.role !== m42Cult.ROLE_CULT_LEADER);
  if (!hideCult) return null;
  if (room && helpers && bot.role === m42Cult.ROLE_CULT_LEADER) {
    const line = m42Bluff.buildCitizenBluffRollCallAnswer(room, bot, helpers);
    if (line) return line;
  }
  return '저는 시민입니다. 조결·취재 결과부터 확인하겠습니다.';
}

function pickSafeReplacement(isMafia) {

  const pool = isMafia ? MAFIA_SAFE_POLICE_LINES : CITIZEN_SAFE_LINES;

  return pool[Math.floor(Math.random() * pool.length)];

}



function pickRoleClaimSafe() {

  return ROLE_CLAIM_SAFE[Math.floor(Math.random() * ROLE_CLAIM_SAFE.length)];

}



function findPlayerByNickname(room, name, helpers) {

  if (!name || !room) return null;

  const players = room.players ? Object.values(room.players) : [];

  return players.find((p) => p.nickname === name)

    || (helpers.getAlivePlayers

      ? helpers.getAlivePlayers(room).find((p) => p.nickname === name)

      : null);

}



function roleLabelToKey(label, helpers) {

  const labels = helpers.ROLE_LABELS || {};

  for (const [key, val] of Object.entries(labels)) {

    if (val === label) return key;

  }

  if (label === '마피아') return 'mafia';

  return null;

}



/** "김무현님은 경찰이었습니다" 등 — 공개 팩트 없으면 차단 */

const SPY_META_LEAK = /접선|밤챗|밤\s*채팅|맢팀|맢표|긁슾|첫접|n접|(?:저|제가|나는).{0,8}스파이|스파이입니다|스파이였|조사\s*중입니다|마피아와\s*접선/;

const SPY_SAFE_LINES = [
  '저는 시민입니다. 조결·기자 공표가 나오면 그걸로 가겠습니다.',
  '아직 공개된 팩트가 부족합니다. 경찰·기자 결과부터 듣겠습니다.',
  '무직 시민입니다. 밤 이야기보다 낮 조사·투표 팩트를 보겠습니다.'
];

/** 접선·밤챗 등 비밀 메타 용어 — 낮 채팅에서 봇이 쓰면 직업 들킴 */
function stripSpyMetaLeak(text, bot, isMafiaTeam) {
  if (!text || !bot) return null;
  const c = text.replace(/\s+/g, '');
  if (!SPY_META_LEAK.test(c) && !SPY_META_LEAK.test(text)) return null;
  if (bot.role === 'politician') {
    return '저는 정치인입니다. 투표로 확인해 주십시오. 조결 후 투표하겠습니다.';
  }
  if (bot.role === 'spy' || (isMafiaTeam && isMafiaTeam(bot.role))) {
    return SPY_SAFE_LINES[Math.floor(Math.random() * SPY_SAFE_LINES.length)];
  }
  return SPY_SAFE_LINES[Math.floor(Math.random() * SPY_SAFE_LINES.length)];
}

/** 시민 직업이 마피아식 맞경 선동 멘트를 쓰는 경우 차단 */
function stripCitizenMatgyeongStir(text, bot, isMafiaTeam) {
  if (!text || !bot) return null;
  if (isMafiaTeam && isMafiaTeam(bot.role)) return null;
  const c = text.replace(/\s+/g, '');
  if (!/맞경입니다|짭경가려|짭경을가려|맞경인데|맞경이면.*짭경/.test(c)) return null;
  if (bot.role === 'politician') {
    return '저는 정치인입니다. 경찰 조결부터 보고 투표하겠습니다.';
  }
  return '맞직이면 조결·취재부터 확인하겠습니다. 저는 시민입니다.';
}

function isMafiaPoliceBluffLine(text, isMafiaTeamFn, bot) {
  if (!text || !bot || !isMafiaTeamFn || !isMafiaTeamFn(bot.role)) return false;
  const c = text.replace(/\s+/g, '');
  return /수사\s*결과|조사\s*결과|(?:저는|나는|제가|홀경|경찰입니다)/.test(c);
}

function stripUnauthorizedRoleClaims(text, room, bot, helpers) {

  if (!text || !room || !helpers) return null;

  if (/수사\s*결과|조사\s*결과/.test(text)) return null;

  if (helpers.isMafiaTeam && helpers.isMafiaTeam(bot.role) && isMafiaPoliceBluffLine(text, helpers.isMafiaTeam, bot)) {
    return null;
  }

  const labels = helpers.ROLE_LABELS || {};

  const roleNames = [...new Set([...Object.values(labels), '마피아'])];

  const roleAlt = roleNames.map((r) => r.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');

  const re = new RegExp(

    `([\\uAC00-\\uD7A3\\w]+)님(?:은|의\\s*직업은?)\\s*(?:\\[)?(${roleAlt})(?:\\])?(?:이었|입니다|였습니다|이었습니다|입니다)`,

    'g'

  );

  let m;

  while ((m = re.exec(text)) !== null) {

    const [, nick, roleLabel] = m;

    if (nick === bot.nickname) continue;

    const player = findPlayerByNickname(room, nick, helpers);

    if (!player) continue;

    const roleKey = roleLabelToKey(roleLabel, helpers);

    if (!roleKey) continue;

    const isPublic = voteFacts.isRolePublicForBot

      ? voteFacts.isRolePublicForBot(room, bot, player.id, roleKey, helpers)

      : false;

    if (!isPublic) return pickRoleClaimSafe();

  }

  return null;

}



/** 도굴·계승 등 비공개 메타를 공개 채팅에 노출하는 멘트 차단 */
function stripMetaInfoLeak(text) {
  if (!text) return null;
  const c = String(text).replace(/\s+/g, '');
  if (/기자를계승|계승했습니다|도굴계승|도굴꾼계승/.test(c)) return null;
  if (/님이기자입니다|기자를계승/.test(c)) return null;
  return text;
}

function sanitizeBotChatLine(text, bot, isMafiaTeam, room = null, helpers = null) {

  if (!text || typeof text !== 'string') return text;

  let trimmed = text.replace(/\s+/g, ' ').trim();
  const metaBlocked = stripMetaInfoLeak(trimmed);
  if (metaBlocked === null) {
    return '오늘은 조결·취재 팩트부터 맞춰 보겠습니다.';
  }
  if (policeFmt.isFormalPoliceReport(trimmed)) {
    trimmed = policeFmt.rewriteFormalPoliceReport(trimmed);
  }

  if (isMafiaTeam && isMafiaTeam(bot.role) && isMafiaPoliceBluffLine(trimmed, isMafiaTeam, bot)) {
    if (!leaksMafia(trimmed)) return trimmed.slice(0, 180);
  }

  const spyBlock = stripSpyMetaLeak(trimmed, bot, isMafiaTeam);
  if (spyBlock) return spyBlock.slice(0, 180);

  const matBlock = stripCitizenMatgyeongStir(trimmed, bot, isMafiaTeam);
  if (matBlock) return matBlock.slice(0, 180);

  const cultBlock = stripCultLeak(trimmed, bot, room, helpers);
  if (cultBlock) return cultBlock.slice(0, 180);

  if (room && helpers) {
    const roleBlock = stripUnauthorizedRoleClaims(trimmed, room, bot, helpers);
    if (roleBlock) return roleBlock.slice(0, 180);
  }

  if (room && helpers && isMafiaTeam && isMafiaTeam(bot.role)) {
    const accuseLike = /맞경|짭경|진경|가짜|거짓|수상|마피아|의심|처형|투표/.test(trimmed);
    if (accuseLike) {
      const allies = Object.values(room.players || {})
        .filter((p) => p && p.alive && p.id !== bot.id)
        .filter((p) => helpers.isMafiaTeam && helpers.isMafiaTeam(p.role))
        .filter((p) => p.nickname && trimmed.includes(p.nickname));
      if (allies.length) {
        const allyHits = allies.map((p) => p.nickname);
        // #region agent log
        agentLog({
          hypothesisId: 'ALLY_CHAT_1',
          location: 'lib/bot-chat-filter.js:sanitizeBotChatLine',
          message: 'evil bot line mentions mafia ally with accuse-like tone',
          runId: 'ally-protect-check',
          data: {
            bot: bot.nickname,
            role: bot.role,
            alliesMentioned: allyHits,
            line: String(trimmed).slice(0, 180)
          }
        });
        // #endregion
        const ally = allies[0];
        return `${ally.nickname}님은 제 관점에서 동료 시민 라인으로 봅니다. 먼저 경찰 조결·기자 공표를 확인합시다.`.slice(0, 180);
      }
    }
  }



  if (!leaksMafia(trimmed)) return trimmed.slice(0, 180);

  const isMafia = isMafiaTeam && isMafiaTeam(bot.role);

  return pickSafeReplacement(isMafia);

}



module.exports = {

  leaksMafia,

  sanitizeBotChatLine,

  pickSafeReplacement,

  stripUnauthorizedRoleClaims

};


