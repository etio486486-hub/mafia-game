...
function ingestReporterPerformer(room, performerId) {
  if (performerId == null) return;
  pushPublicVoteIntel(room, {
    targetId: performerId,
    role: 'reporter',
    source: 'reporter_performer'
  });
}
...
function ingestReporterReveal(room, reveal, botLearnRole, reporterBotId = null) {
  ...
  if (reporterBotId != null) ingestReporterPerformer(room, reporterBotId);
...
function ingestSoldierSpyCatch(room, catchData, botLearnRole) {
  if (!catchData || catchData.spyId == null) return;
  pushPublicVoteIntel(room, {
    targetId: catchData.spyId,
    role: 'spy',
    isMafia: true,
    source: 'soldier_spy_catch'
  });
  if (catchData.soldierId != null) {
    pushPublicVoteIntel(room, {
      targetId: catchData.soldierId,
      role: 'soldier',
      source: 'soldier_spy_catch_performer'
    });
    syncBotKnownRolePrivate(room, catchData.soldierId, catchData.spyId, 'spy', botLearnRole);
  }
}
...
function ingestFromNightReport(room, report, botLearnRole) {
  ...
  if (report.soldierSpyCatch) {
    ingestSoldierSpyCatch(room, report.soldierSpyCatch, botLearnRole);
  }
}

/** 채팅에 올라온 수사 결과 → 봇 투표 팩트 (조결 후 자투 방지) */
function ingestMediumPerformer(room, performerId) {
  if (performerId == null) return;
  pushPublicVoteIntel(room, {
    targetId: performerId,
    role: 'medium',
    source: 'medium_performer'
  });
}

function ingestClericCultExpose(room, leaderId, clericId, botLearnRole) {
  if (leaderId == null) return;
  pushPublicVoteIntel(room, {
    targetId: leaderId,
    role: 'cult_leader',
    source: 'cleric_expose'
  });
  if (clericId != null) {
    syncBotKnownRolePrivate(room, clericId, leaderId, 'cult_leader', botLearnRole);
  }
}
...
function ingestMediumPurifyReveal(room, targetId, role, botLearnRole, mediumBotId = null) {
  ...
  if (!target || target.alive || !mediumPurify.isMediumPurifyEligible(room, target)) return;
  pushPublicVoteIntel(room, {
    targetId,
    role,
    source: 'medium'
  });
  if (mediumBotId != null) ingestMediumPerformer(room, mediumBotId);
  if (mediumBotId) syncBotKnownRolePrivate(room, mediumBotId, targetId, role, botLearnRole);
}

/** 사망자 채팅에서 영매가 전달한 의심 대상 */
function ingestMediumDeadChatSuspect(room, suspectId, mediumBotId) {
  if (!suspectId) return;
  pushPublicVoteIntel(room, {
    targetId: suspectId,
    isMafia: true,
    source: 'medium_dead_chat'
  });
  if (mediumBotId != null) ingestMediumPerformer(room, mediumBotId);
}
...
function ingestMediumPurifyFromDayChat(room, helpers, roleLabels) {
  ...
  for (const msg of dayChat) {
    ...
    ingestMediumPerformer(room, msg.fromId);
    const speaker = helpers.getPlayerById ? helpers.getPlayerById(room, msg.fromId) : null;
    const mediumBotId = speaker && speaker.isBot && speaker.role === 'medium' ? speaker.id : msg.fromId;
...
function ingestPrivateDetectiveReportsFromDayChat(room, helpers) {
  if (!room.game) return;
  const m42Pd = require('./m42-private-detective');
  const dayChat = helpers.getChatMessages
    ? helpers.getChatMessages(room, 'day')
    : (room.chatLog && room.chatLog.day) || [];
  if (!room.game.ingestedPrivateDetectiveChatKeys) {
    room.game.ingestedPrivateDetectiveChatKeys = {};
  }

  for (const msg of dayChat) {
    if (!msg?.text || !msg.fromId || msg.system) continue;
    if (!m42Pd.looksLikeDetectiveBrief(msg.text, room)) continue;
    const parsed = m42Pd.parseDetectiveReportFromText(room, msg.text);
    if (!parsed.watch) continue;

    const speaker = helpers.getPlayerById ? helpers.getPlayerById(room, msg.fromId) : null;
    if (!m42Pd.isTrustedPrivateDetectiveSpeaker(room, speaker, helpers)) continue;
    if (!m42Pd.reportMatchesNightIntel(room, msg.fromId, parsed)) continue;

    const key = `${msg.fromId}:${msg.time || 0}:${msg.text.slice(0, 96)}`;
    if (room.game.ingestedPrivateDetectiveChatKeys[key]) continue;
    room.game.ingestedPrivateDetectiveChatKeys[key] = true;

    pushPublicVoteIntel(room, {
      targetId: speaker.id,
      role: 'private_detective',
      source: 'private_detective_performer',
      isMafia: false
    });

    for (const row of m42Pd.toVoteIntelRows(parsed)) {
      pushPublicVoteIntel(room, {
        targetId: row.targetId,
        isMafia: row.isMafia,
        role: row.role,
        source: 'private_detective'
      });
    }
  }
}
...
function ingestPoliceReportsFromDayChat(room, helpers) {
  ...
  for (const msg of dayChat) {
    if (!msg?.text || !msg.fromId || msg.system) continue;
    const { innocent, mafia } = voteFacts.parsePoliceReportFromText(room, msg.text);
    const looksReport = policeFmt.looksLikePoliceReport(msg.text, room)
      || policeFmt.hasSubstantivePoliceVerdict(msg.text, room)
      || voteFacts.isPoliceReportProviding(msg.text, room);
    if (!looksReport && !innocent.length && !mafia.length) continue;

    const speaker = helpers.getPlayerById ? helpers.getPlayerById(room, msg.fromId) : null;
    if (!m42Bluff.isTrustedHolgyeongPoliceSpeaker(room, speaker, helpers)) continue;
...
module.exports = {
  ...
  ingestSoldierBlock,
  ingestSoldierSpyCatch,
  ingestFromNightReport,
  ingestMediumPerformer,
  ingestClericCultExpose,
  ingestMediumPurifyReveal,
  ingestMediumDeadChatSuspect,
  ingestMediumPurifyFromDayChat,
  ingestReporterPerformer,
  ingestChatMafiaAccusationsFromDayChat,
  ingestPoliceReportsFromDayChat,
  ingestPrivateDetectiveReportsFromDayChat,
