...
function ingestMediumPurifyReveal(room, targetId, role, botLearnRole) {
  ...
  syncBotKnownRoleAll(room, targetId, role, botLearnRole);
}
...
function ingestMediumPurifyFromDayChat(room, helpers, roleLabels) {
  ...
  for (const msg of dayChat) {
    ...
    ingestMediumPurifyReveal(room, parsed.targetId, parsed.role, helpers.botLearnRole);
  }
}
