...
function pickSkillChatReaction(brief, bot, topic, ctx, helpers) {
  ...
  switch (topic) {
    ...
    case 'police':
      if (bot.role === 'police') {
        const policeLines = [
          '저는 경찰입니다. 밤 수사한 사람만 조결로 말하고, 시민은 그 조결로 투표합시다.',
          '조결은 제가 수사한 결과만 공개합니다. 맞경이면 조결부터 맞춰 봅시다.',
          '경찰은 시민 편입니다. 조결·취재·성불 맞는 쪽으로 같이 지목하겠습니다.',
          '홀경 조결은 무시하고, 제 밤 수사 결과를 기준으로 가겠습니다.',
          '맞경·맞의·맞군·맞영이면 한쪽은 맢일 확률이 큽니다. 조사·성불·취재로 가립시다.'
        ];
