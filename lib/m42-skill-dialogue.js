...
function pickDawnReaction(brief, bot, nightReport, helpers) {
  ...
  if (nightReport.healSave && !isMafia) {
    if (role === 'doctor' && nightReport.doctorBotId === bot.id) {
      return '밤에 치료는 했습니다. 대상은 공개하지 않겠습니다.';
    }
  ...
  if (deaths.length) {
    if (isMafia) {
      return deathNames
        ? `밤에 ${deathNames}님이 사망했습니다. ${top}님 쪽부터 살펴보겠습니다.`
        : '밤에 사망이 있었습니다. 저는 시민입니다. 함께 찾아보겠습니다.';
    }
    if (role === 'medium') {
      return `${deathNames}님 사망자 채팅도 확인해 주십시오. 성불 단서가 있을 수 있습니다.`;
    }
    return `${deathNames}님 사망을 확인했습니다. ${top}님부터 질문하겠습니다.`;
  }

  if (nightReport.quietNight) {
    if (isMafia) {
      return `조밤입니다. ${top}님 행적이 더 수상합니다.`;
    }
    if (role === 'doctor') {
      return '조용한 밤입니다. 치료할 대상이 없었거나 이미 막힌 밤일 수 있습니다.';
    }
...
function pickSkillChatReaction(brief, bot, topic, ctx, helpers) {
  ...
  switch (topic) {
    ...
    case 'doctor':
      if (bot.role === 'doctor') {
        return '저는 의사입니다. 누굴 살렸는지는 밤마다 다릅니다. 직공만 믿어 주십시오.';
      }
      if (isMafia) {
        return '의사가 있으면 밤에 막힐 수 있습니다. 다른 각을 잡겠습니다.';
      }
      return '힐이 떴으면 의사일 수 있습니다. 조밤이면 치료 성공일 수도 있습니다.';
...
function pickSelfSkillFollowUp(brief, bot, nightReport) {
  ...
  if (act.type === 'police' && act.isMafia === false) {
    return `밤에 ${act.targetName}님을 수사했습니다. 조결 요청 시 말씀드리겠습니다.`;
  }
  if (act.type === 'police' && act.isMafia === true) {
    return '수사 기록은 제게만 있습니다. 공개는 신중히 하겠습니다.';
  }
  if (act.type === 'spy') {
  ...
  if (act.type === 'doctor') {
    return `밤에 ${act.targetName}님을 치료했습니다. 대상은 비밀입니다.`;
  }
  if (act.type === 'medium') {
