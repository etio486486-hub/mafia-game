...
function hasReporterScoopInDayChat(room, helpers) {
  const msgs = m42Pd.getDayChatMessages(room, helpers);
  return msgs.some(
    (m) =>
      m?.text
      && /기자\s*취재|취재\s*공표|오늘\s*아침\s*공표|아침\s*공표|기자\s*공표/.test(String(m.text))
  );
}

function hasReporterSelfSpokenInDayChat(room, reporterId, helpers) {
...
function pickDawnReaction(brief, bot, nightReport, helpers, room) {
  ...
  const pdPublicInChat = room && m42Pd.hasPrivateDetectiveBriefInDayChat(room, helpers);

  if (pd && pdPublicInChat) {
  ...
  const medTarget = medReveal?.targetId && helpers.getPlayerById
    ? helpers.getPlayerById(room, medReveal.targetId)
    : null;
  const medRevealOk = medReveal && medTarget && !medTarget.alive;
  if (medRevealOk) {
  ...
  const soldierCatch = nightReport.soldierSpyCatch;
  if (soldierCatch && soldierCatch.spyName) {
    if (role === 'soldier' && nightReport.soldierSpyCatchSoldierId === bot.id) {
      return `밤에 ${soldierCatch.spyName}님이 조사하다 들켰습니다. 낮에 스파이 신원을 공표하겠습니다.`;
    }
    if (isMafia) {
      return `${soldierCatch.spyName}님 스파이 이야기는 일단 넘기고, 조결·취재부터 맞추겠습니다.`;
    }
    if (role === 'police') {
      return `군인이 ${soldierCatch.spyName}님을 스파이로 검거했다는 말이 있습니다. 조결과 대조하겠습니다.`;
    }
    return `군인 검거로 ${soldierCatch.spyName}님이 스파이로 지목됐습니다. 투표 근거로 정리하겠습니다.`;
  }
  ...
  switch (topic) {
    ...
    case 'medium': {
      ...
      const medTarget = medReveal?.targetId && helpers.getPlayerById
        ? helpers.getPlayerById(room, medReveal.targetId)
        : null;
      const medOk = medReveal && medTarget && !medTarget.alive;
      if (bot.role === 'medium') {
        if (medOk) {
...
function pickSoldierSpyCatchRevealLine(room, soldier, catchData) {
  if (!soldier || !catchData?.spyName) return null;
  const lines = [
    `스파이가 나에게 손댔는데 들켰다. 스파이는 ${catchData.spyName}님입니다.`,
    `밤에 ${catchData.spyName}님이 조사하다 걸렸습니다. 저는 군인이고, 스파이는 ${catchData.spyName}님입니다.`,
    `${catchData.spyName}님이 밤에 제게 손댔는데 잡았습니다. 스파이는 ${catchData.spyName}님입니다.`
  ];
  return lines[Math.floor(Math.random() * lines.length)];
}

function pickSoldierSpyCatchCitizenLine(room, bot, catchData) {
  if (!bot || !catchData?.spyName) return null;
  const spyName = catchData.spyName;
  const soldierName = catchData.soldierName || '군인';
  const lines = [
    `${soldierName}님 말대로 ${spyName}님이 스파이로 보입니다. 오늘 ${spyName}님부터 몰표합시다.`,
    `군인 검거 확인했습니다. ${spyName}님 스파이 — 마피아 팀 쪽으로 투표 모읍시다.`,
    `${spyName}님 스파이 지목이 맞다면 마피아·스파이 라인부터 처분해야 합니다.`,
    `스파이 ${spyName}님부터 의심하겠습니다. 경찰 조결·취재와도 맞는지 봅시다.`
  ];
  if (bot.role === 'police') {
    return `군인이 ${spyName}님을 스파이로 검거했습니다. 제 조결과 대조한 뒤 ${spyName}님 쪽 투표를 모으겠습니다.`;
  }
  return lines[Math.floor(Math.random() * lines.length)];
}

function pickReporterRevealDayLine(brief, bot, reveal, helpers, room) {
  if (!reveal) return null;
  const role = bot.role;
  const isMafia = brief.isMafia;
  const top = brief.topSuspect;
  const matProb = require('./m42-matgyeong-probability');
  const bluffMod = require('./m42-bluff');
  const alivePoliceId = room && bluffMod.getAlivePoliceId
    ? bluffMod.getAlivePoliceId(room, helpers)
    : null;
  const policeConfirmedDead = reveal.role === 'police' && !alivePoliceId;

  if (policeConfirmedDead && room) {
    const reporters = bluffMod.scanPoliceReporters(room, helpers).filter((r) => r.id !== bot.id);
    const fakeName = reporters[0]
      ? (helpers.getPlayerById ? helpers.getPlayerById(room, reporters[0].id)?.nickname : null)
      : null;
    const suspect = fakeName || top || '맞경';
    if (isMafia) {
      return [
        `기자 공표로 ${reveal.targetName}님이 경찰이었습니다. 생존 맞경 ${suspect}님 조결·타임라인부터 의심하겠습니다.`,
        `${reveal.targetName}님 [경찰] 확인. 남은 경찰 주장 ${suspect}님 쪽이 짭경일 확률이 큽니다. 저는 시민입니다.`
      ][Math.floor(Math.random() * 2)];
    }
    const stir = matProb.pickCitizenProbabilityLine(room, bot, helpers);
    if (stir) return stir;
    return `기자 취재로 ${reveal.targetName}님이 경찰이었습니다. 진경 없는데 ${suspect}님 맞경은 마피아·스파이일 가능성이 큽니다. 오늘 ${suspect}님부터 몰표합시다.`;
  }
...
module.exports = {
  SKILL_CHAT,
  hasReporterScoopInDayChat,
  hasReporterSelfSpokenInDayChat,
  isRevealMafia,
  detectSkillTopic,
  pickDawnReaction,
  ...
  pickSoldierSpyCatchRevealLine,
  pickSoldierSpyCatchCitizenLine
};
