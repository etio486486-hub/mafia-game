...
function validateClericNightTarget(room, cleric, targetId, helpers) {
  ...
  const base = canClericReviveTarget(room, target);
  if (!base.ok) return base;
  if (
    helpers
    && voteFacts.isPublicMafiaSuspectTarget(room, targetId, helpers, cleric)
  ) {
    return {
      ok: false,
      message: '마피아로 의심·확정된 플레이어는 부활할 수 없습니다.'
    };
  }
  return base;
}
...
function applyClericReviveAtDayStart(room, ctx) {
  ...
  if (
    ctx.voteFactHelpers
    && voteFacts.isPublicMafiaSuspectTarget(room, target.id, ctx.voteFactHelpers, cleric)
  ) {
    g.pendingClericRevive = null;
    return null;
  }
  ...
  g.pendingClericRevive = null;
  if (g.pendingMediumReveal?.targetId === target.id) {
