...
const ROLE_INTEL = {
  mafia: {
    ...
    day: '낮 내내 시민 연기+지속 블러핑. 홀경·맞경·짭경·쓰리경 선동, 거짓 직공, 가짜 조결을 반복. 경찰 2명·맞직 유도 후 짭경 몰기. 밤챗·마피아 노출 금지.',
    claim: ['시민', '군인', '의사'],
  ...
  spy: {
    ...
    day: '낮 공개 채팅은 무직 시민 연기+지속 블러핑. 가짜 조결·맞경 선동. 접선·밤챗·스파이 노출 금지.',
    claim: ['시민', '경찰'],
    counters: ['police'],
  ...
  doctor: {
    ...
    day: '눈힐(직공 없이 치료). 의사는 특수직이라 직공하지 않음. 경찰 직공·조결이 나오면 그 추리를 따라가고 경찰 보호 우선.',
    claim: [],
    counters: [],
  ...
  politician: {
    team: 'citizen',
    night: '밤 행동 없음.',
    day: '낮 투표 2표·찬반 면역. 봇은 "투표로 확인" 한 줄 위주.',
    claim: ['정치인'],
    counters: [],
    slang: ['2표', '면역']
  },
  ...
  private_detective: {
    team: 'citizen',
    night: '밤에 생존자 1명을 지정해 그 사람이 누구에게 손(능력)을 뻗는지 관찰. 경찰·의사·마피아 등 가능성 멘트.',
    day: '관찰 결과는 발언과 조결·취재로 교차검증. 직공은 신중히.',
    claim: ['사립탐정'],
    counters: ['mafia'],
    slang: ['사탐', '관찰', '손방향']
  },
  ...
  cult_leader: {
    team: 'cult',
    night: '홀수 밤 생존자 1명 포교. 마피아 지목 시 실패.',
    day: '낮 공개 채팅은 마피아팀처럼 지속 블러핑(거짓 직공·가짜 조결·맞경 선동). 교주·포교·종소리·신도·교주팀 절대 금지. 시민팀 직업으로 헷갈리게. 중복 직공 허용.',
    claim: ['시민', '경찰', '군인', '기자', '영매', '정치인'],
    counters: [],
    slang: ['종소리', '포교']
  },
  cleric: {
    team: 'citizen',
    night: '1회 사망자 부활(다음 날). 교주 포교 시도 시 포교 실패·교주 정체 확인. 교주 진영 불가.',
    day: '교주에게 포교당한 밤이면 낮에 교주 정체를 공표. 부활 정보는 팩트로만.',
    claim: [],
    counters: [],
    slang: ['부활', '기적']
  },
  terrorist: {
    team: 'citizen',
    night: '없음. 마피아 킬 시 산화(공격 마피아 동반 사망).',
    day: '처형 시 동귀어진 1명 지정.',
    claim: [],
    counters: [],
    slang: ['자폭', '산화', '동귀어진']
  ...
  beast_man: {
    team: 'mafia',
    night: '마피아 킬 시 접선. 마피아 전멸 후 밤 처형.',
    day: '경찰에게 시민으로 보임. 접선 전 마피아 모름.',
    claim: ['시민'],
    counters: [],
    slang: ['접선', '갈망', '짐승']
  },
  cultist: {
    team: 'cult',
    night: '교주 알고 시작. 포교 시 즉시 합류. 교주 사망 시 계승.',
    day: '교주팀 연기. 교주·광신도 노출 금지.',
    claim: ['시민'],
    counters: [],
    slang: ['숭배', '신앙']
  }
...
const OPEN_BY_PHASE = {
  ...
  day_vote: [
    () => '이제 투표입니다. 조결·취재 팩트 없으면 무투·자투 쪽이 낫습니다.',
    () => '근거 없는 몰표는 하지 않겠습니다. 애매하면 자투 또는 무투로 넘기겠습니다.',
    (b) => b.topSuspect && b.topSuspect !== '누군가'
      ? `${b.topSuspect}님은 공개 팩트가 있을 때만 지목하겠습니다.`
      : '팩트가 없으면 무투·자투로 가겠습니다.'
  ]
};
...
function buildSituationBrief(room, bot, ctx, helpers) {
  ...
  return {
    ...
    debateHint,
    isMafia: intel.team === 'mafia' || bot.role === 'cult_leader',
    isEvilBluff: intel.team === 'mafia' || bot.role === 'cult_leader' || !!bot.joinedCult,
...
function pickRoleAwareLine(brief, bot, targetName, speaker, room, helpers) {
  ...
  if (role === 'politician') {
    if (/투표|처형/.test(brief.triggerText || '')) {
      return bot.isBot
        ...
        : '저는 낮 투표 2표이고 찬반 처형은 면역입니다. 근거 보고 결정해 주세요.';
    }
  }
  ...
  if (role === 'doctor' && /힐|치료|의사/.test(brief.triggerText || '')) {
    return '의사는 특수직입니다. 저는 직공하지 않고 눈힐이 낫습니다. 다른 분 추리·조결을 따르겠습니다.';
  }
  ...
  if (role === 'medium' && /성불|영매|사망/.test(brief.triggerText || '')) {
    const dead = (brief.deadPlayers || []).filter(Boolean);
    if (dead.length && /성불/.test(brief.triggerText || '')) {
      return `영매입니다. ${dead[0]}님 등 의심 사망자부터 성불하겠습니다.`;
    }
  ...
  if (brief.isEvilBluff && room && helpers) {
    if (Math.random() < 0.78) {
  ...
  if (intel.team === 'citizen' && brief.knownMafia.length) {
    return `공개된 조결·취재상 ${brief.knownMafia[0]}님 쪽이 마피아로 보입니다.`;
  }
...
function suggestLastWords(brief, bot) {
  ...
  const suspect = brief.topSuspect && brief.topSuspect !== '누군가' ? brief.topSuspect : null;
  const alt = suspect || (brief.knownMafia && brief.knownMafia[0]) || null;

  if (bot.role === 'politician') {
    const pol = [
      '저 정치인입니다. 찬반 면역이니 투표로 확인해 주십시오.',
      '정치인 본인입니다. 투표로 검증해 주십시오.'
    ];
    return pol[Math.floor(Math.random() * pol.length)];
  }
  if (bot.role === 'police') {
    return alt
      ? `저는 경찰입니다. ${alt}님 쪽이 더 수상합니다. 저를 죽이면 시민이 손해입니다.`
      : '저는 경찰입니다. 조결은 수사한 사람만 말할 수 있습니다. 저를 죽이면 시민에게 손해입니다.';
  }
  if (brief.isMafia) {
    const lines = [
      `저는 시민입니다. 근거 없이 ${name}을 처형하면 마피아 이득입니다.`,
      alt
        ? `${alt}님 발언·투표가 더 수상합니다. 저는 무고합니다.`
        : '조결·취재 없이 저만 몰표하면 투갈 납니다. 다른 사람부터 보십시오.',
      suspect
        ? `${suspect}님 쪽이 더 수상한데 왜 저입니까? 저는 마피아가 아닙니다.`
        : '억울합니다. 팩트 나올 때까지 무근거 지목은 하지 마십시오.'
    ];
    return lines[Math.floor(Math.random() * lines.length)];
  }
  if (alt) {
    return `억울합니다. ${alt}님 쪽이 더 수상한데 왜 저입니까? 저는 시민입니다.`;
  }
  return '억울합니다. 조사·취재 팩트 없이 저만 밀면 시민이 손해입니다.';
}
...
function suggestExecutionVote(brief, bot, candidate, helpers = {}) {
  ...
  if (brief.knownMafia.includes(candidate.nickname)) return 'yes';
  return 'no';
}
...
function buildLlmSystemPrompt(provider = 'openai') {
  ...
  return [
    ...
    '듀얼카드는 없습니다. 교주는 낮에 시민팀 직업으로만 연기하고 교주·포교를 말하지 말 것.',
    `진행: ${PHASE_FLOW.join(' → ')}`,
    ...
    '자신이 의심·지목당했을 때 "억울합니다. 자투로 넘기겠습니다" 같은 짧은 멘트 금지. 왜 억울한지 1문장 + 다른 수상 인물과 근거 1문장으로 답할 것.',
    ...
    '마피아팀·교주·신도(낮 공개채팅): 매 턴 지속 블러핑. 거짓 직공, 가짜 조결, 홀경·맞경·짭경·쓰리경 선동으로 시민을 헷갈리게. 교주·포교·밤챗·접선 노출 금지.',
    '위장 정치인은 "정치인입니다·2표·면역"을 밝힌 뒤 맢 흉내로 표를 모을 수 있으나, 채팅에 없는 직업 팩트로 타인을 단정하지 말 것. 위장 경찰은 처음 경찰 블러핑을 택하면 끝까지 경찰 조결 톤으로 우길 것.',
    '마피아팀 예: "맞경입니다. ○○, △△ 중 짭경 가립시다. 저는 경찰입니다." "○○님 조사했는데 마피아 아닙니다." 맞경이면 "진경은 나", "상대 조결 거짓", "밤에 내가 찍은 사람이 다르다"처럼 서로 우기는 멘트를 반복.',
    '경찰 제거·의심 말은 "시민에 도움", "조결 필요" 식으로만. "마피아가 경찰을 죽여야" 같은 표현 금지.',
    '「각자 직업」에는 위장 직업으로 1문장 직공. 이미 맞경이면 위장 경찰·시민·군인 중 하나로 일관되게.',
    '진짜 의사는 절대 의사 직공하지 말 것(마피아가 먼저 죽임). 의사는 특수직이라고 설명하되 본인은 시민·무직으로만 말할 것. 의사인 척 직공하는 건 마피아팀·교주팀만.',
    '마피아팀·교주팀(시민 위장): "일반 시민 외 특수직은 직공 못 한다", "무직 시민" 멘트 사용. 의사 위장 시 "의사는 특수직"이라고 말할 것.',
    '스파이 등 특직은 밤 행동·대상 공개 금지. 마피아·스파이는 절대 진짜 직업을 밝히지 말 것.',
    '다른 플레이어 직업은 채팅·조결·기자·영매 성불 등 공개된 팩트만 앎. 게임 시작 시 서로 직업을 모르는 것처럼 연기할 것.',
    '낮 투표: 조결·취재 등 강한 팩트 없으면 무투·자투(자기 투표). 근거 없는 몰표·다수 몰아처형 금지.',
    fluid
...
function buildLlmUserPrompt(brief, bot, task) {
  ...
  return JSON.stringify({
    ...
    yourRole: bot.role,
    team: labels.team,
    evilBluff: !!brief.isEvilBluff,
    phase: brief.phase,
    day: brief.dayIndex,
    night: brief.nightIndex,
    alive: brief.aliveCount,
    dawn: brief.dawnLine,
    knownMafiaFromPublicChat: brief.knownMafia,
    knownCitizenFromPublicChat: brief.knownCitizen,
    deadPlayers: brief.deadPlayers || [],
    recentChat: brief.recentChat || '',
    lastSpeaker: brief.lastSpeaker || '',
    lastMessage: brief.lastMessage || '',
    replyRule: '반드시 직전 발화(lastSpeaker/lastMessage)에 이어 받으며, 이미 채팅에 나온 조결·직공·조밤 이야기를 무시하고 같은 질문을 반복하지 말 것.',
    triggerFrom: brief.triggerText || '',
    nightGuide: labels.night,
    dayGuide: labels.day,
    roleSlang: brief.roleSlangHint,
    matchedTerms: brief.matchedGlossary,
    bluffHint: brief.isEvilBluff ? m42Bluff.buildLlmBluffHint(brief, bot) : '',
