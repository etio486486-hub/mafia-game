...
const DAWN_HINTS = [
  { pattern: /조용|사망자는 없/, line: '조밤이네요. 은폐·물총·치료 성공일 수 있어요.' },
  { pattern: /사망/, line: '밤에 사망이 있었어요. 퍼블·연퍼 여부부터 봅시다.' },
  { pattern: /기자/, line: '기자 취재 나왔으면 그 정보부터 정리합시다.' }
];

const OPEN_BY_PHASE = {
  day_chat_early: [
    (b) => `${b.dayIndex}일차입니다. ${b.dawnLine || '어젯밤 정리부터 해볼까요?'}`,
    (b) => `남은 토론 ${b.debateHint}. 직공·조결 필요하면 말해 주세요.`
  ],
  day_chat_mid: [
    (b) => `지금 ${b.topSuspect}님이 제일 수상해요.`,
    (b) => `투표 전에 ${b.topSuspect}님 발언 좀 더 들어봐야 할 것 같아요.`
  ],
  day_vote: [
    () => '이제 투표입니다. 몰표보다 근거 있는 지목이 낫습니다.',
    () => '딱히 없으면 자투로 넘기는 것도 방법이에요.'
  ]
};
...
function pickRoleAwareLine(brief, bot, targetName, speaker) {
  ...
  if (role === 'police' && brief.triggerText && /조결|경찰/.test(brief.triggerText)) {
    return '수사한 사람만 말씀드릴게요. 조결은 허위 없이 공개합니다.';
  }
  ...
  if (role === 'reporter' && brief.nightIndex < 2) {
    return '아직 2밤 전이라 취재는 못 했어요. 그 전까지는 추리로 가요.';
  }

  if (role === 'doctor' && /힐|치료|의사/.test(brief.triggerText || '')) {
    return '저 의사인데 누굴 살렸는지는 밤마다 달라요. 조밤이면 치료 성공일 수도 있습니다.';
  }
  ...
  if (role === 'soldier' && /군인|방탄/.test(brief.triggerText || '')) {
    return '군인이에요. 방탄은 한 번뿐입니다.';
  }

  if (role === 'spy' && !brief.joinedMafiaChat) {
    return `저는 ${targetName}님부터 지켜볼게요. 접선 전이라 정보가 부족해요.`;
  }

  if (isMafia && knownMafia.includes(targetName)) {
    return `${speaker ? `${speaker}님, ` : ''}${targetName}님 말은 일단 넘어가고 다른 쪽 봅시다.`;
  }

  if (isMafia) {
    const lines = [
      `${targetName}님이 좀 수상한데요.`,
      `조밤이었으니 ${targetName}님부터 질문해볼까요?`,
      `자투로 가도 되는 날인데 ${targetName}님만 왜 급해요?`
    ];
    return lines[Math.floor(Math.random() * lines.length)];
  }

  if (intel.team === 'citizen' && brief.knownMafia.length) {
    return `${brief.knownMafia[0]}님 쪽이 마피아 느낌입니다.`;
  }

  if (brief.quietNight && !isMafia) {
    return `조밤이었어요. ${targetName}님 행적부터 확인합시다.`;
  }
...
function suggestLastWords(brief, bot) {
  ...
  if (bot.role === 'police') {
    return '경찰입니다. 조결은 수사한 사람만 말할 수 있어요. 저 죽이면 시민 손해예요.';
  }
  if (brief.isMafia) {
    const lines = [
      '저 진짜 시민이에요. 투갈 나오면 마피아 이득이에요.',
      `${name}이 아니라 다른 사람 보세요. 몰표 주의하세요.`,
      '억울합니다. 자투로 넘기죠.'
    ];
    return lines[Math.floor(Math.random() * lines.length)];
  }
  return `억울해요. ${brief.topSuspect}님 쪽이 더 수상한데 왜 저인가요?`;
}
...
function suggestExecutionVote(brief, bot, candidate, helpers = {}) {
  ...
  if (candidate.nickname === brief.topSuspect && brief.topScore >= 3) return 'yes';
...
function buildLlmSystemPrompt() {
  return [
    ...
    '한국어 1~2문장만. 욕설·메타(봇/AI) 금지. 생존자만 지목. 사망자 이름 금지.',
