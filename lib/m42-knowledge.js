...
const ROLE_INTEL = {
  mafia: {
    ...
    day: '시민인 척 거짓 직공(홀경·맞경 블러핑). 맞경이면 짭경·늦경 몰기, 가끔 둘째 경찰 연기. 경찰·의사·기자 제거 우선. 조밤이면 물총·치료 의심을 다른 쪽으로.',
    claim: ['시민', '군인', '의사'],
  ...
  spy: {
    ...
    day: '접선 전 시민·군인 위장. 접선 후에도 낮엔 시민처럼, 맞경·맞직에 끼어 짭경 찾기 연기.',
    claim: ['시민', '경찰'],
    counters: ['police'],
...
function pickRoleAwareLine(brief, bot, targetName, speaker) {
  ...
  if (isMafia) {
    ...
    return '조결이 나오면 그 기준으로 말하겠습니다. 무근거 지목은 하지 않겠습니다.';
  }
...
function buildLlmSystemPrompt(provider = 'openai') {
  ...
  return [
    ...
    '마피아 팀·스파이는 낮에만 시민 연기. "마피아", "밤챗", "우리 팀", "맢팀" 노출 금지.',
    '블러핑(마피아·스파이만): 1~2낮에 홀경·가짜 직공. 맞경·짭경 선동은 채팅에 경찰 직공이 2명 이상 나온 뒤에만.',
    '시민 직업(정치인·군인·의사·기자·영매·일반시민)은 맞경이라며 선동·짭경 몰기 금지. 정치인은 2표·찬반면역만 말할 것.',
    '마피아팀 맞경 예(직공 2명 이후): "조결 보고 짭경 가려야 합니다." 가짜 경찰이면 "저는 진경입니다."',
