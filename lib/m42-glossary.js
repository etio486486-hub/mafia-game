...
const ENTRIES = [
  ...
  { id: 'sabtam', aliases: ['사립탐정', '사탐', '관찰'], meaning: '밤에 한 생존자가 누구에게 능력(손)을 쓰는지 방향을 관찰하는 시민 직', cat: 'police', roles: ['private_detective'] },
...
function pickGlossaryReaction(brief, bot, text) {
  ...
  if (ids.has('heal') || ids.has('jaheal') || ids.has('taheal')) {
    if (bot.role === 'doctor') {
      return '의사는 특수직입니다. 힐 여부는 말하지 않고 조결·취재를 따르겠습니다.';
    }
    return '의사는 특수직입니다. 힐이 떴다고 의사라고 단정하진 않겠습니다.';
  }
